...
pub fn pick_group_key_strategy(types: &[DataType]) -> GroupKeyStrategy {
    if types.is_empty() {
...
pub fn can_apply_fixed_size_key(types: &[DataType]) -> bool {
...
pub fn can_apply_compressed_key(types: &[DataType]) -> bool {
    if types.len() < 2 {
        return false;
    }
...
pub fn is_compressible_key_type(data_type: &DataType) -> bool {
    matches!(
        data_type,
        DataType::Int8
            | DataType::Int16
...
pub fn fixed_width_size(data_type: &DataType) -> Option<usize> {
