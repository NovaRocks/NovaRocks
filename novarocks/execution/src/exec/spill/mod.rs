...
pub use crate::runtime::spill_config::{SpillConfig, SpillMode};
...
impl QuerySpillManager {
    pub fn new(
        ...
        execution_runtime: &crate::runtime::execution_runtime::ExecutionRuntime,
    ) -> Self {
        let profile = profiler.map(SpillProfile::new);
        Self {
            config,
            channel: SpillChannelHandle::new_with_limits(
                execution_runtime.config().spill_io_threads,
                execution_runtime.config().spill_io_queue_capacity,
            ),
            profile,
        }
    }
