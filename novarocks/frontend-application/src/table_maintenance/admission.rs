...
mod semantic_tests {
    ...
    fn semantic_maintenance(source: &str) -> MaintenanceSqlCommand {
    ...
    #[test]
    fn lowers_semantic_maintenance_commands() {
        assert_eq!(
            lower_semantic_maintenance_statement(
                &semantic_maintenance(
                    "CALL ice.system.rewrite_manifests(table => 'analytics.orders', use_caching => TRUE, spec_id => 3)",
                ),
                context(),
            ),
            Ok(ParsedMaintenanceStatement::Execute {
                name_parts: vec!["ice".into(), "analytics".into(), "orders".into()],
                action: ParsedMaintenanceAction::RewriteManifests {
        ...
        assert_eq!(
            lower_semantic_maintenance_statement(
                &semantic_maintenance(
                    "ALTER TABLE ice.analytics.orders EXPIRE SNAPSHOTS OLDER THAN 1700000000000 RETAIN LAST 3",
                ),
                context(),
            ),
            Ok(ParsedMaintenanceStatement::Execute {
    ...
    #[test]
    fn lowers_semantic_show_optimize_presentation() {
        let command = semantic_maintenance(
            "SHOW ALTER TABLE OPTIMIZE FROM `ice`.`analytics` \
             WHERE `TableName` = 'orders' ORDER BY `CreateTime` DESC LIMIT 20",
        );
        let MaintenanceSqlCommand::ShowOptimize(show) = command else {
            panic!("expected semantic SHOW ALTER TABLE OPTIMIZE command");
        };

        assert_eq!(
            lower_semantic_show_optimize(&show),
            Ok(ParsedShowOptimize {
        ...
        let error = lower_semantic_show_optimize(
            match &semantic_maintenance("SHOW ALTER TABLE OPTIMIZE WHERE TableName = 1") {
                MaintenanceSqlCommand::ShowOptimize(show) => show,
                _ => panic!("expected semantic SHOW ALTER TABLE OPTIMIZE command"),
            },
        )
        .expect_err("numeric TableName filter is not part of maintenance admission");
        assert!(error.contains("WHERE TableName"));
    }

    #[test]
    fn semantic_maintenance_rejects_invalid_admission_values() {
        for (source, expected) in [
            ...
            let error =
                lower_semantic_maintenance_statement(&semantic_maintenance(source), context())
                    .expect_err("invalid semantic maintenance input must fail");
