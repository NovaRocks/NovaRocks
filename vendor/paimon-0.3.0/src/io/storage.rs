...
impl Storage {
    pub(crate) fn build(file_io_builder: FileIOBuilder) -> crate::Result<Self> {
        ...
        match scheme.as_str() {
            ...
            #[cfg(feature = "storage-cos")]
            "cos" | "cosn" => {
                let config = super::cos_config_parse(_props)?;
                Ok(Self::Cos {
                    config: Box::new(config),
                    operators: Mutex::new(HashMap::new()),
                })
            ...
            "abfs" | "abfss" | "az" | "azdfs" | "azdls" | "azure" => {
                let config = super::azdls_config_parse(_props)?;
                Ok(Self::Azdls {
                    config: Box::new(config),
                    operators: Mutex::new(HashMap::new()),
                })
            ...
            #[cfg(feature = "storage-obs")]
            "obs" => {
                let config = super::obs_config_parse(_props)?;
                Ok(Self::Obs {
                    config: Box::new(config),
                    operators: Mutex::new(HashMap::new()),
                })
            ...
            #[cfg(feature = "storage-gcs")]
            "gcs" | "gs" => {
                let config = super::gcs_config_parse(_props)?;
                Ok(Self::Gcs {
                    config: Box::new(config),
                    operators: Mutex::new(HashMap::new()),
                })
            ...
            "hdfs" | "hdfs-native" | "hdfs_native" => {
                let config = super::hdfs_config_parse(_props)?;
                Ok(Self::Hdfs {
                    config: Box::new(config),
                    op: Mutex::new(None),
                })
    ...
    pub(crate) fn create<'a>(&self, _path: &'a str) -> crate::Result<(Operator, Cow<'a, str>)> {
        #[allow(unreachable_patterns)]
        match self {
            #[cfg(feature = "storage-memory")]
            Storage::Memory { op } => Ok((
                op.clone(),
                Cow::Borrowed(Self::memory_relative_path(_path)?),
            )),
            #[cfg(feature = "storage-fs")]
            Storage::LocalFs { op } => Ok((op.clone(), Self::fs_relative_path(_path)?)),
            #[cfg(feature = "storage-oss")]
            Storage::Oss { config, operators } => {
                let (bucket, relative_path) =
                    Self::bucket_and_relative_path(_path, "OSS", &["oss"])?;
                let op = Self::cached_oss_operator(config, operators, _path, &bucket)?;
                Ok((op, Cow::Borrowed(relative_path)))
            }
            #[cfg(feature = "storage-s3")]
            Storage::S3 { config, operators } => {
                let (bucket, relative_path) =
                    Self::bucket_and_relative_path(_path, "S3", &["s3", "s3a"])?;
                let op = Self::cached_s3_operator(config, operators, _path, &bucket)?;
                Ok((op, Cow::Borrowed(relative_path)))
            }
            #[cfg(feature = "storage-cos")]
            Storage::Cos { config, operators } => {
                let (bucket, relative_path) =
                    Self::bucket_and_relative_path(_path, "COS", &["cos", "cosn"])?;
                let op = Self::cached_operator(operators, "COS", &bucket, || {
                    super::cos_config_build(config, _path)
                })?;
                Ok((op, Cow::Borrowed(relative_path)))
            }
            #[cfg(feature = "storage-azdls")]
            Storage::Azdls { config, operators } => {
                let relative_path = super::azdls_relative_path(_path)?;
                let cache_key = super::azdls_operator_cache_key(config, _path)?;
                let op = Self::cached_operator(operators, "Azure", &cache_key, || {
                    super::azdls_config_build(config, _path)
                })?;
                Ok((op, Cow::Borrowed(relative_path)))
            }
            #[cfg(feature = "storage-obs")]
            Storage::Obs { config, operators } => {
                let (bucket, relative_path) =
                    Self::bucket_and_relative_path(_path, "OBS", &["obs"])?;
                let op = Self::cached_operator(operators, "OBS", &bucket, || {
                    super::obs_config_build(config, _path)
                })?;
                Ok((op, Cow::Borrowed(relative_path)))
            }
            #[cfg(feature = "storage-gcs")]
            Storage::Gcs { config, operators } => {
                let (bucket, relative_path) =
                    Self::bucket_and_relative_path(_path, "GCS", &["gcs", "gs"])?;
                let op = Self::cached_operator(operators, "GCS", &bucket, || {
                    super::gcs_config_build(config, _path)
                })?;
                Ok((op, Cow::Borrowed(relative_path)))
            }
            #[cfg(feature = "storage-hdfs")]
            Storage::Hdfs { config, op } => {
                let relative_path = super::hdfs_relative_path(_path)?;
                ...
                // the first _path's NameNode if not set in config. One FileIO
                // instance should target exactly one HDFS cluster.
                if guard.is_none() {
                    *guard = Some(super::hdfs_config_build(config, _path)?);
                }
                Ok((
            ...
            _ => Err(error::Error::IoUnsupported {
                message: "No SDK-managed storage feature is enabled".to_string(),
            }),
        }
    }
