...
pub fn eval_array_contains_seq(
    ...
    let left = left_arr
        ...
        .ok_or_else(|| {
            format!(
                "array_contains_seq expects ListArray, got {:?}",
                left_arr.data_type()
            )
        })?;
    let right = right_arr
    ...
    if left_values.data_type() != right_values.data_type() {
        ...
        if let Ok(casted) =
            super::common::cast_with_special_rules(&right_values, &left_type, "array_contains_seq")
        {
            right_values = casted;
        } else if let Ok(casted) =
            super::common::cast_with_special_rules(&left_values, &right_type, "array_contains_seq")
        {
            left_values = casted;
        } else {
            return Err(format!(
                "array_contains_seq type mismatch after coercion attempts: {:?} vs {:?}",
    ...
    for row in 0..chunk.len() {
        ...
        let contains_seq = if right_len == 0 {
            true
        } else if right_len > left_len {
            false
        } else {
            let mut found = false;
            for pos in 0..=(left_len - right_len) {
