...
use arrow::array::{Array, ArrayRef, UInt32Array, new_empty_array, new_null_array};
use arrow::compute::{cast, take};
use arrow::datatypes::DataType;
use arrow::record_batch::RecordBatch;

use crate::agent_service::{TAlterJobType, TAlterTabletReqV2, TTabletType};
...
pub(crate) fn execute_alter_tablet_task(request: &TAlterTabletReqV2) -> Result<(), String> {
    ...
    let base_metadata =
        load_tablet_metadata_at_version(&base_root_path, base_tablet_id, alter_version)?
            .ok_or_else(|| {
                format!(
                    "alter task base metadata not found: tablet_id={} alter_version={}",
                    base_tablet_id, alter_version
                )
            })?;
    let new_metadata = load_tablet_metadata_at_version(&new_root_path, new_tablet_id, 1)?
        .ok_or_else(|| {
            format!(
                "alter task new tablet base metadata not found: tablet_id={} version=1",
                new_tablet_id
            )
        })?;
    ...
    let base_read_schema = if let Some(read_schema) = request.base_tablet_read_schema.as_ref() {
        ...
        base_metadata.schema.clone().ok_or_else(|| {
            format!(
                "alter task base tablet metadata missing schema: tablet_id={} version={}",
                base_tablet_id, alter_version
            )
        })?
    };
    let new_metadata_schema = new_metadata.schema.clone().ok_or_else(|| {
        format!(
            "alter task new tablet metadata missing schema: tablet_id={} version=1",
            new_tablet_id
        )
    })?;
    tracing::info!(
    ...
    for (rowset_idx, source_rowset) in base_metadata.rowsets.iter().enumerate() {
        ...
        let transformed =
            transform_rowset_batch(&source_batch, &base_read_schema, &new_schema, rowset_idx)?;
...
fn validate_schema_change_request(request: &TAlterTabletReqV2) -> Result<(), String> {
    ...
    if alter_job_type != TAlterJobType::SCHEMA_CHANGE {
        return Err(format!(
            "alter task unsupported alter_job_type={alter_job_type:?} (only SCHEMA_CHANGE is supported)"
        ));
    }
    if request
    ...
    if request.materialized_column_req.is_some() {
        return Err(
            "alter task does not support materialized_column_req in SCHEMA_CHANGE V1".to_string(),
        );
    }
    if request.where_expr.is_some() {
        return Err("alter task does not support where_expr in SCHEMA_CHANGE V1".to_string());
    }
    Ok(())
}
