-- Test Objective:
-- 1. Verify correctness of ARRAY, MAP, STRUCT defaults across all primitive types and precisions.
-- 2. Verify field order preservation for STRUCT defaults (non-alphabetical field names).
-- 3. Verify deep nesting combinations (3-6 levels) of ARRAY/MAP/STRUCT defaults.
-- 4. Verify sub-element access (indexing, field access) from default values.
-- 5. Verify special characters and unicode in complex type defaults.
-- 6. Verify subfield pruning optimization with complex defaults.
...
-- query 20
-- Section 20: Accessing Sub-Elements from Default Values - ARRAY elements
USE ${case_db};
SELECT
    id,
...
FROM ${case_db}.array_primitives
ORDER BY id;

-- query 21
-- Section 20 continued: Nested ARRAY elements
USE ${case_db};
SELECT
    id,
...
FROM ${case_db}.nested_arrays
ORDER BY id;

-- query 22
-- Section 20 continued: MAP values by key
USE ${case_db};
SELECT
    id,
...
FROM ${case_db}.map_types
ORDER BY id;

-- query 23
-- Section 20 continued: STRUCT fields
USE ${case_db};
SELECT
    id,
...
FROM ${case_db}.struct_field_order
ORDER BY id;

-- query 24
-- Section 20 continued: Nested STRUCT fields
USE ${case_db};
SELECT
    id,
...
-- query 25
-- Section 20 continued: Elements in ARRAY<STRUCT>
USE ${case_db};
SELECT
    id,
...
FROM ${case_db}.array_struct
ORDER BY id;

-- query 26
-- Section 20 continued: Elements in MAP<K, ARRAY>
USE ${case_db};
SELECT
    id,
...
FROM ${case_db}.map_array
ORDER BY id;

-- query 27
-- Section 20 continued: Elements in MAP<K, STRUCT>
USE ${case_db};
SELECT
    id,
...
FROM ${case_db}.map_struct
ORDER BY id;

-- query 28
-- Section 20 continued: Deeply nested elements (cross-table join)
USE ${case_db};
SELECT
    d1.id,
...
FROM ${case_db}.deep_nesting_1 d1, ${case_db}.deep_nesting_2 d2
ORDER BY d1.id;

-- query 29
-- Section 20 continued: DECIMAL256 in nested structures
USE ${case_db};
SELECT
    id,
...
FROM ${case_db}.decimal256_nested
ORDER BY id;
...
-- query 31
-- Section 21 continued: Outer mid subfield and deeper subfield x
USE ${case_db};
SELECT
  id,
  st_outer.mid,
...
-- query 32
-- Section 21 continued: Only the deepest sub field
USE ${case_db};
SELECT
  id,
  st_outer.mid.sub
...
-- query 33
-- Section 22: Subfield prune on nested complex defaults - STRUCT with ARRAY<STRUCT<nested STRUCT>>
USE ${case_db};
...
-- query 34
-- Section 22 continued: Query second element meta y
USE ${case_db};
SELECT
  id,
...
-- query 35
-- Section 22 continued: Query map value struct subfields and nested array element
USE ${case_db};
SELECT
  id,
  st_mix.mp[1].m AS m1,
...
-- query 36
-- Section 22 continued: Query whole map and a pruned leaf together
USE ${case_db};
SELECT
  id,
  st_mix.mp AS mp_all,
...
-- query 37
-- Section 22 continued: Mix scalar field and deep nested field
USE ${case_db};
SELECT
  id,
  st_mix.name AS name,
...
-- query 38
-- Section 22 continued: ARRAY -> MAP -> STRUCT -> nested STRUCT
USE ${case_db};
...
-- query 39
-- Section 22 continued: Query outer subfield and deeper subfield together
USE ${case_db};
SELECT
  id,
  arr_mp[1][1] AS v1,
...
-- query 40
-- Section 22 continued: MAP -> ARRAY<STRUCT<nested STRUCT>>
USE ${case_db};
...
-- query 41
-- Section 22 continued: Query outer array element struct and deeper leaf field together
USE ${case_db};
SELECT
  id,
  mp_arr['k'][1] AS e1,
