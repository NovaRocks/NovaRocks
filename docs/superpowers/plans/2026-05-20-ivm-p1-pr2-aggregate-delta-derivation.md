# IVM-P1 PR2 — Aggregate-delta affected partition derivation

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a partition derivation API that takes signed aggregate delta chunks plus the MV schema contract / layout and returns the set of affected target partitions, using `iceberg::transform::create_transform_function` for all first-class transforms. Strict fail-fast on contract/layout/transform/value errors. No production caller is wired up in PR2 — that happens in PR3.

**Architecture:** Introduce a new module `src/engine/mv/partition/aggregate_delta.rs` next to the existing PR2 (`planner.rs`) and PR1 (`mapping.rs`) modules. New types: `AggregateDeltaPartitionInput`, `AffectedAggregateTargetPartitions { Unpartitioned, Known { partitions } }`, `AffectedPartitionError`. Implementation walks each `MvPartitionFieldContract`, resolves the target → output → group-key column via existing `MvSchemaContract` + `AggregateMvLayout` indexing, looks up the corresponding Arrow column in the delta chunk, runs `iceberg::transform::create_transform_function(&Transform).transform(array)`, then serialises every transformed row into `MvPartitionValue::{Null, String}` using the same primitive-display rules that `change_partition_value` uses on the manifest side (cross-path equality is locked in by a property-style unit test).

**Tech Stack:** Rust, Iceberg-rust 0.9 (`iceberg::spec::Transform`, `iceberg::transform::TransformFunction`), Arrow `ArrayRef`, NovaRocks `AggregateMvLayout`, `MvSchemaContract`, `cargo test --lib` for fast unit tests.

---

## File Structure

- Create: `src/engine/mv/partition/aggregate_delta.rs`
  - Exposes `AggregateDeltaPartitionInput`, `AffectedAggregateTargetPartitions`, `AffectedPartitionError`, `derive_from_aggregate_delta`.
  - Holds the contract → iceberg transform translator and the Arrow-row → `MvPartitionValue` serialiser, both kept private to this module unless needed elsewhere.
- Modify: `src/engine/mv/partition/mod.rs`
  - Re-export the public types of the new module.

PR2 introduces no production caller; existing files (`iceberg_refresh.rs`, `iceberg_aggregate_state.rs`, etc.) are not touched.

---

## Task 1: Add the result + error types
...
Create `src/engine/mv/partition/aggregate_delta.rs` and put this test module at the bottom:
...
Add this `mod.rs` change in `src/engine/mv/partition/mod.rs`:
...
cargo test --lib engine::mv::partition::aggregate_delta
```

Expected: compile fails because `aggregate_delta.rs` body is empty.

- [ ] **Step 3: Implement the result + error types**

Replace the top of `src/engine/mv/partition/aggregate_delta.rs` (above the `#[cfg(test)]`) with:
...
impl std::fmt::Display for AffectedPartitionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::GroupKeyColumnMissing { field, reason } => write!(
                f,
                "aggregate target partition field {field}: {reason}"
            ),
...
```bash
git add src/engine/mv/partition/aggregate_delta.rs src/engine/mv/partition/mod.rs
git commit -m "feat: add aggregate delta partition derivation result and error types"
```

---

## Task 2: Add the contract-transform → iceberg-transform translator

**Files:**
- Modify: `src/engine/mv/partition/aggregate_delta.rs`

`iceberg::transform::create_transform_function` needs an `iceberg::spec::Transform`. The contract carries `MvPartitionTransformContract` (a re-encoded enum that excludes `Unknown`). The translator returns `Err(AffectedPartitionError::TransformUnsupported)` for `Void` because void partitions provide no pruning value and should never reach derivation; all other variants map 1:1.

- [ ] **Step 1: Write the failing tests**

Append to the `#[cfg(test)] mod tests` block in `src/engine/mv/partition/aggregate_delta.rs`:
...
```bash
cargo test --lib engine::mv::partition::aggregate_delta::tests::contract_transform_to_iceberg
```

Expected: compile fails because `contract_transform_to_iceberg` does not exist.

- [ ] **Step 3: Implement the translator**

Append to `src/engine/mv/partition/aggregate_delta.rs` (above the test module):
...
```bash
git add src/engine/mv/partition/aggregate_delta.rs
git commit -m "feat: translate contract partition transform to iceberg transform enum"
```

---

## Task 3: Add the Arrow-row → `MvPartitionValue` serialiser with cross-path equality test

**Files:**
- Modify: `src/engine/mv/partition/aggregate_delta.rs`

To keep the file-metadata path (`change_partition_value`) and the new delta-chunk path producing equal `MvPartitionValue::String` content for the same logical value, the serialiser hand-walks the same Iceberg primitive set the file-metadata path covers. The unit test below feeds the same Iceberg `PrimitiveLiteral` through both paths and asserts string equality.

The serialiser supports the Arrow types Iceberg transforms produce on output: `BooleanArray`, `Int32Array`, `Int64Array`, `Float32Array`, `Float64Array`, `StringArray`, `Decimal128Array`, `Date32Array`, and the four `TimestampXxxxArray` widths. For unsupported Arrow types the serialiser returns `GroupKeyTypeMismatch`.
...
```bash
cargo test --lib engine::mv::partition::aggregate_delta::tests::arrow_row_to_partition_value
cargo test --lib engine::mv::partition::aggregate_delta::tests::client_side_serialization_matches_file_metadata_path
```

Expected: compile fails because `arrow_array_row_to_partition_value` does not exist.

- [ ] **Step 3: Implement the serialiser**

Append to `src/engine/mv/partition/aggregate_delta.rs` (above the test module, below `contract_transform_to_iceberg`):
...
```bash
cargo test --lib engine::mv::partition::aggregate_delta::tests::arrow_row_to_partition_value
cargo test --lib engine::mv::partition::aggregate_delta::tests::arrow_row_to_partition_value_handles_null
cargo test --lib engine::mv::partition::aggregate_delta::tests::arrow_row_to_partition_value_rejects_unsupported_arrow_type
cargo test --lib engine::mv::partition::aggregate_delta::tests::client_side_serialization_matches_file_metadata_path_for_primitive_literals
```
...
```bash
git add src/engine/mv/partition/aggregate_delta.rs
git commit -m "feat: serialize arrow partition values to mv partition value"
```

---

## Task 4: Implement `derive_from_aggregate_delta`

**Files:**
- Modify: `src/engine/mv/partition/aggregate_delta.rs`

Now wire everything together. The function:

1. Returns `Unpartitioned` when `schema_contract.target.partition.is_none()`. (No contract = nothing to derive. No layout cross-check is required because `apply_iceberg_aggregate_delta_chunks` is the only caller — it ALWAYS pairs aggregate contract with aggregate layout, and PR1 has already proved aggregate layout has at least one group key column.)
2. For each `MvPartitionFieldContract` in the contract:
   - Resolve `partition_field.source_target_field_id` → `output_index` via `contract.target.visible_columns`.
   - Read `contract.output.columns[output_index].expression`; require `kind == Column` and `referenced_base_field_ids.len() == 1`; otherwise `OutputLineageNotPureColumn`.
   - Verify the output_index is one of `layout.group_key_source_indexes`; otherwise `OutputLineageNotPureColumn` (defense in depth — DDL is supposed to prevent this).
   - Resolve `layout.visible_columns[output_index].name` as the Arrow column name in each delta chunk.
   - Convert the contract transform to `iceberg::spec::Transform` (Task 2 helper).
3. Walk every delta chunk:
   - Look up the column index for each partition field's name via `chunk.batch.schema().index_of(name)`. Failure → `GroupKeyColumnMissing`.
   - For each partition field, call `iceberg::transform::create_transform_function(&transform).map_err(...)?.transform(array)` once per chunk; surface as `TransformFailed` on error.
4. For every row in each chunk: build `MvPartitionKey` with `spec_id = contract.target.partition.target_spec_id`, one `MvPartitionKeyField` per partition field (`field_name` from contract, value via Task 3 helper). Insert into `BTreeSet`.
5. Return `Known { partitions: set }`.

Empty delta chunks (all `num_rows == 0`) produce an empty Known set.
...
```rust
    ...
    fn count_layout_with_group_key(name: &str, data_type: DataType, sql_type: SqlType) -> AggregateMvLayout {
        let row_id = managed_physical_column("__row_id__".to_string(), SqlType::String, false, false, true);
    ...
    fn batch_with_group_key(name: &str, dt: DataType, values: ArrayRef) -> Chunk {
        let n = values.len();
    ...
    fn derive_day_transform_normalizes_dates_to_day_buckets() {
        ...
        let chunk = batch_with_group_key(
            "ts",
            DataType::Date32,
            Arc::new(Date32Array::from(vec![Some(19500), Some(19501), Some(19500)])) as ArrayRef,
        );
    ...
    fn derive_bucket_transform_uses_iceberg_hash() {
        ...
        let chunk = batch_with_group_key(
            "region",
            DataType::Utf8,
            Arc::new(StringArray::from(vec![Some("east"), Some("west")])) as ArrayRef,
        );

        // Independently compute the expected bucket values via iceberg-rust
        // and assert the derivation produced exactly those.
        let arr: ArrayRef = Arc::new(StringArray::from(vec![Some("east"), Some("west")]));
        let xform = iceberg::transform::create_transform_function(
            &iceberg::spec::Transform::Bucket(8),
        )
        .expect("transform");
    ...
    fn derive_missing_target_field_returns_group_key_missing() {
        let layout = count_layout_with_group_key("region", DataType::Utf8, SqlType::String);
        // Contract references a target_field_id that has no visible column.
        let mut contract = count_contract_with_partition(
            "region",
            MvPartitionTransformContract::Identity,
            999, // does not match any visible column
        );
        contract
            .target
            .partition
    ...
    fn derive_non_pure_output_lineage_returns_error() {
        ...
        contract.output.columns[0].expression.kind = ExpressionKind::Function;
        contract.output.columns[0].expression.referenced_base_field_ids = vec![1, 2];
    ...
    fn derive_missing_chunk_column_returns_group_key_missing() {
        ...
        let row_ids: ArrayRef = Arc::new(StringArray::from(vec![Some("rid-0")]));
        let other: ArrayRef = Arc::new(StringArray::from(vec![Some("a")]));
        let counts: ArrayRef = Arc::new(Int64Array::from(vec![1i64]));
        let states: ArrayRef = Arc::new(Int64Array::from(vec![1i64]));
        let schema = Arc::new(Schema::new(vec![
            Field::new("__row_id__", DataType::Utf8, false),
...
```bash
cargo test --lib engine::mv::partition::aggregate_delta::tests::derive
```

Expected: compile fails because `AggregateDeltaPartitionInput` and `derive_from_aggregate_delta` do not exist.

- [ ] **Step 3: Implement the input struct and the derivation**

Append to `src/engine/mv/partition/aggregate_delta.rs` (above the test module, after the helpers):
...
pub(crate) fn derive_from_aggregate_delta(
    ...
    for partition_field in &partition.fields {
        ...
        let column = input.layout.visible_columns.get(output_index).ok_or_else(|| {
    ...
    let mut partitions: std::collections::BTreeSet<MvPartitionKey> =
    ...
    for chunk in input.delta_chunks {
        ...
        // Apply each partition field's transform to its source column once
        // per chunk; collect the transformed arrays.
...
Expected: every test (around 13 in total counting Tasks 1–4) passes.

- [ ] **Step 5: Commit**

```bash
git add src/engine/mv/partition/aggregate_delta.rs
git commit -m "feat: derive affected target partitions from aggregate delta chunks"
```
...
- [ ] **Step 2: Lint (PR2-touched files only)**

```bash
cargo clippy --all-targets --no-deps -- -D warnings 2>&1 | grep -E "(error|warning):.*src/engine/mv/partition/aggregate_delta\.rs|src/engine/mv/partition/mod\.rs" || echo "OK"
```

Expected: `OK` (no clippy output for the two new/modified files).
...
Expected: all partition module tests pass (existing 24 + PR2's new aggregate_delta tests).
...
Expected: compile succeeds. No production caller of `derive_from_aggregate_delta` exists in PR2; this just confirms nothing else broke.
...
- [ ] **Step 6: Commit any fmt-only changes**

If `cargo fmt` produced changes after Task 4's commit:

```bash
git status -sb
git add src/engine/mv/partition/aggregate_delta.rs src/engine/mv/partition/mod.rs
git commit -m "style: format aggregate delta partition derivation"
```
...
**Spec coverage** (each spec §13 PR2 deliverable → plan Task):

| Spec deliverable | Plan Task |
|---|---|
| `partition::aggregate_delta::derive_from_aggregate_delta` | Task 4 |
| `AggregateDeltaPartitionInput`, `AffectedAggregateTargetPartitions`, `AffectedPartitionError` | Tasks 1 + 4 |
| Contract transform → iceberg transform | Task 2 |
| Arrow row → `MvPartitionValue` (with same primitive rules as `change_partition_value`) | Task 3 |
| Cross-path equality test (spec §12.1.4 property-style) | Task 3 |
| Tests §12.1.1 (identity / day / bucket / void / contract-missing-target-field / unpartitioned / non-pure / missing chunk column / empty chunks) | Task 4 |
| Fail-fast on every error variant | Tasks 1–4 |

**Placeholder scan:** No `TBD`, `TODO`, `implement later`, `similar to Task N`. Every step has full code. No `unimplemented!()` stubs.

**Type consistency:**
- `AggregateDeltaPartitionInput` shape is identical across Tasks 4 and §6.1 of the source spec (`layout`, `schema_contract`, `delta_chunks`).
- `AffectedPartitionError` variants referenced in Tasks 1, 2, 3, 4 match: `ContractMissing`, `TransformUnsupported`, `OutputLineageNotPureColumn`, `GroupKeyColumnMissing`, `GroupKeyTypeMismatch`, `TransformFailed`.
- `contract_transform_to_iceberg` and `arrow_array_row_to_partition_value` signatures stay consistent between definition (Tasks 2, 3) and use (Task 4).
